...
public class Ex7 extends JFrame {
    ...
    private JTable tableStudent, tableClass, tableMetadata, tableStatistics;
    private DefaultTableModel modelStudent, modelClass, modelMetadata, modelStatistics;
    private JTextField txtID, txtName, txtClass, txtScore;
    private JTextField txtClassID, txtClassName, txtClassTeacher;
    
    private JComboBox<String> cbSearchType, cbStatisticsType;
    ...
    public Ex7() {
        setTitle("Ex7: Complete Student Management System with Statistics");
        setSize(1000, 780);
    ...
    private JPanel createStatisticsPanel() {
        ...
        topPanel.setBorder(BorderFactory.createTitledBorder("Select Report Type"));

        String[] reports = {
            "1. Count students in each class",
            "2. Class with the most students",
            "3. Class with the highest average GPA",
            "4. Class with the most failed students (GPA < 2.0)"
        };
        cbStatisticsType = new JComboBox<>(reports);
        topPanel.add(cbStatisticsType);

        tableStatistics = new JTable();
        modelStatistics = new DefaultTableModel();
        tableStatistics.setModel(modelStatistics);
        JScrollPane scrollPane = new JScrollPane(tableStatistics);

        panel.add(topPanel, BorderLayout.NORTH);
        panel.add(scrollPane, BorderLayout.CENTER);

        cbStatisticsType.addActionListener(e -> loadStatisticsReport());
        
        return panel;
    }

    private void loadStatisticsReport() {
        int index = cbStatisticsType.getSelectedIndex();
        String sql = "";
        String[] headers = null;

        switch (index) {
            case 0:
                sql = "SELECT Lop, COUNT(MaSV) AS 'Student Count' FROM SinhVien GROUP BY Lop";
                headers = new String[]{"Class Name", "Student Count"};
                break;
            case 1:
                sql = "SELECT TOP 1 Lop, COUNT(MaSV) AS 'Student Count' FROM SinhVien GROUP BY Lop ORDER BY COUNT(MaSV) DESC";
                headers = new String[]{"Class Name", "Student Count"};
                break;
            case 2:
                sql = "SELECT TOP 1 Lop, AVG(DiemTB) AS 'Average GPA' FROM SinhVien GROUP BY Lop ORDER BY AVG(DiemTB) DESC";
                headers = new String[]{"Class Name", "Average GPA"};
                break;
            case 3:
                sql = "SELECT TOP 1 Lop, COUNT(MaSV) AS 'Failed Count' FROM SinhVien WHERE DiemTB < 2.0 GROUP BY Lop ORDER BY COUNT(MaSV) DESC";
                headers = new String[]{"Class Name", "Failed Count"};
                break;
        }
        ...
        try (Connection conn = DriverManager.getConnection(dbURL, dbUser, dbPassword);
            ...
            modelStatistics.setDataVector(data, columnNames);
        } catch (SQLException e) {
            JOptionPane.showMessageDialog(this, "Statistics Error: " + e.getMessage());
        }
    }
